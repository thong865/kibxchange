<script setup>
const { pending, data: crptoData } = await useLazyAsyncData("crptoData", () =>
  ...
  <div>
    ...
    <div class="containerx">
      <div>
        <div>ราคาตลาด</div>
        <div
          class="grid grid-cols-4 gap-4 drop-shadow-md bg-sky-300/[0.15] text-white py-2 px-3"
        >
        ...
        <div
          class="grid grid-cols-4 gap-4 mt-1 rounded-md px-2 py-2 drop-shadow-md bg-sky-300/[0.15] text-white text-sm"
          v-for="i in 20"
          :key="i"
        >
          <div class="flex gap-2">
            <img src="/ctosymbol/BTC.svg" class="w-8" />
            <div>Bitcoin BTC</div>
          </div>
          <div class="...">
            {{
              (34552.3232894643).toLocaleString("en-us", {
        ...
        <!-- <div class="grid grid-cols-4 gap-4" v-for="(dt,ind) in crptoData" :key="ind">
        <div class="...">{{dt.name}}</div>
        <div class="...">{{dt.quote['USD'].price}}</div>
        <div class="...">{{dt.quote['USD'].volume_change_24h}}</div>
        <div class=""></div>
      </div> -->
      </div>
      <div>
